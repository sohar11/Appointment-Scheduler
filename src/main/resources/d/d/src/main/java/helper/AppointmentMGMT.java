    ...
    public static ObservableList<Appointment> pullAllAppointments() {
        try {
            ...
            while(result.next())
            {
                ...
                LocalDateTime start = timeMGMT.convertUTCToSystem(result.getTimestamp("Start").toLocalDateTime()) ;
                LocalDateTime end = timeMGMT.convertUTCToSystem(result.getTimestamp("End").toLocalDateTime());
